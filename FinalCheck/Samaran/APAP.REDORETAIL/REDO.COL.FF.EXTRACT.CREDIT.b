* @ValidationCode : Mjo3Mzc2NzM1OTk6Q3AxMjUyOjE2ODE4ODgxMDY0MDc6SVRTUzotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 19 Apr 2023 12:38:26
...
* Product Code, this code is used to report TRACE info
    ...
    CALL APAP.TAM.REDO.R.COL.GET.MAPPING(C.ID.STATIC.MAPPING, R.STATIC.MAPPING, 1, R.STATIC.MAPPING, "TIPO.INV", Y.TMPCREDITOCODIGOTIPOINVERSION);* R22 Manual conversion - CALL method format changed
    IF E NE "" THEN
        RETURN
    END
...
* Due Days & Number of due payments
    ...
    CALL APAP.REDOSRTN.REDO.S.COL.GET.BILL.DETAILS(AA.ID,R.AA.ACCOUNT.DETAILS,  Y.PROCESS.DATE,  Y.MORA.CTA.VEN, Y.TMPCREDITODIASATRASO, Y.TMPCREDITOCUOTASVENCIDAS, Y.TMPCREDITOCUOTASPAGADAS, Y.TMPCREDITOMONTOMOROSO );* R22 Manual conversion - CALL method format changed
* Total
...
* Payment Frequency (M,W,etc)
    ...
    END
* Contract Status
    CALL APAP.AA.REDO.S.COL.GET.AA.STATUS(AA.ID, Y.PROCESS.DATE, Y.TMPCREDITOESTADOCONTRATO, Y.TMPCREDITOCOBROJUDICIAL);* R22 Manual conversion - CALL method format changed
    IF E THEN
        RETURN
    END
...
*-----------------------------------------------------------------------------
    ...
    CALL APAP.REDOBATCH.REDO.B.COL.FF.EXT.LAST.PAY(AA.ID,Y.TOTAL.CUOTAS,Y.LASTPAY.AMT, Y.LASTPAY.DAT,Y.NEXTPAY.AMT,Y.NEXTPAY.DATE,Y.AL.PAID.BILLS.CNT);* R22 Manual conversion - CALL method format changed
