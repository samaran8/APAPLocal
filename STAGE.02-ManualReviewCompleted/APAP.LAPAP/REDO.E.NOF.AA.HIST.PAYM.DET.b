* @ValidationCode : MjotMTczMTcxMjQwOTpDcDEyNTI6MTY4MjA3NzAwNzQ5NDozMzNzdTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 21 Apr 2023 17:06:47
...
* Date: 21/02/2019
* Description:
*  - Reajuste del balance pendiente
*  - Exclusion de la actividad INCLUSION DE VALORES EN CUOTA
*  - Inclusion de las actividades de AJUSTE DE CUOTA que afecten ACCOUNT
* ---------------
*Autor: Oliver Fermin
*Date: 14/05/2019
*Description:
*  - Reajuste a las consultas al AA.ACTIVITY.HISTORY.HIST para préstamos cancelados
*  - Exclusión de la actividad LENDING-ISSUEBILL-REPAYMENT.SCHEDULE*SOLO.INTERES
*  - Ajuste para préstamos cancelados al momento de consultar el ARRANGEMENT asociado a la cuenta en la tabla FBNK.ACCOUNT$HIS.
*-----------------------------------------------------------------------------------
*Modification History:
*DATE                 WHO                  REFERENCE                     DESCRIPTION
*21/04/2023      CONVERSION TOOL     AUTO R22 CODE CONVERSION        FM TO @FM, VM TO @VM, SM TO @SM,++ TO +=, -- TO -=,F.READ TO CACHE.READ, INCLUDE TO INSERT
...
*-----------------------------------------------------------------------------
    ...
    $INSERT I_F.STMT.ENTRY ;*AUTO R22 CODE CONVERSION - END
...
*Ajuste para préstamos cancelados.

    IF NOT (Y.ARR.ID) THEN

        SEL.CMD.ARR = " SELECT FBNK.ACCOUNT$HIS ARRANGEMENT.ID WITH @ID LIKE " :Y.ARR.ID:"...":" AND ARRANGEMENT.ID NE '' SAMPLE 1"
        CALL EB.READLIST(SEL.CMD.ARR, SEL.LIST.ARR, '',NO.OF.RECS,SEL.ERR)

        Y.ARR.ID = SEL.LIST.ARR;
...
***************************************************************
    ...
    WHILE YACT.ID:AA.POSN
        ...
        IF YEFF.DATE GE Y.START.DATE AND YEFF.DATE LE Y.END.DATE THEN
            ...
            CALL CACHE.READ(FN.AA.ACTIVITY, YACTIVITY, R.AA.ACTIVITY, AA.ACTIVITY.ERR) ;*AUTO R22 CODE CONVERSION
            ...
            IF YACTIVITY EQ 'LENDING-TAKEOVER-ARRANGEMENT' THEN
                ...
                CALL APAP.TAM.REDO.L.GET.DISBURSEMENT.DETAILS(Y.ARR.ID,R.DISB.DETAILS,Y.COMMITED.AMT,Y.PEND.DISB) ;*MANUAL R22 CODE CONVERSION
...
********************
    ...
    Y.ACHIS = ''
*SEL.CMD = " SELECT " : FN.AA.ACTIVITY.HISTORY.HIS : " WITH @ID LIKE " :Y.ARR.ID:"...":" BY EFFECTIVE.DATE"
*CALL EB.READLIST(SEL.CMD, SEL.LIST, '',NO.OF.RECS,SEL.ERR)

    SEL.LIST = SORT(YACT.ID.HIST);
...
**********
    ...
    IF (Y.PRIN.BL AND Y.PRIN.BL NE 0) OR (Y.INT.BL AND Y.INT.BL NE 0) OR (Y.OT.BL AND Y.OT.BL NE 0) OR (Y.INS.BL AND Y.INS.BL NE 0) OR (YMORA.BL AND YMORA.BL NE 0) OR (YINSUR.BL AND YINSUR.BL NE 0) THEN
        ...
        IF YACTIVITY NE 'LENDING-DISBURSE-COMMITMENT' AND YACTIVITY NE 'LENDING-APPLYPAYMENT-RP.CARGOS.DES' THEN
            ...
            PAGO.DTE = PAGO.DTE[7,2]:"/":PAGO.DTE[5,2]:"/":PAGO.DTE[1,4]
...
* FECHA DE PAGO  SUCU  TRANSACCION     PAGO TOTAL  CAPITAL       INTERES      VENCIDO     MORA         SEGURO         CARGOS      CAPITAL PENDIENTE
            ...
            FIN.YPAGO.TOT += YPAGO.TOT ;*AUTO R22 CODE CONVERSION - START
            FINY.PRIN.BL += Y.PRIN.BL
            FINY.INT.BL += Y.INT.BL
            FINY.OT.BL += Y.OT.BL
            FINY.INS.BL += Y.INS.BL
            FIN.YMORA.BL += YMORA.BL
            FIN.YINSUR.BL += YINSUR.BL ;*AUTO R22 CODE CONVERSION -END
        END
