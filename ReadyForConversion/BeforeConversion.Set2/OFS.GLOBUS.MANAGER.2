...
*      IF OFS$GLOBMAN.ACTIVE > 0 THEN
    IF OFS$GLOBMAN.ACTIVE > 0 AND OFS$SOURCE.ID = K.OFS.SOURCE THEN
* EN_10001999 E
* Handle recursive calls to the same application.
        IF APPLICATION = FIELD(OFS.MESSAGE,COMP.SEP,1) THEN
            OFS$GLOBMAN.ACTIVE = 0
            RETURN
        END

        IN.QUEUE.DIR = OFS$SOURCE.REC<OFS.SRC.IN.QUEUE.DIR>
        CONVERT "\" TO "/" IN IN.QUEUE.DIR
...
* GLOBUS_BG_100004988 E
    ...
    IF SOURCE.TYPE.ORG = "TELNET" OR SOURCE.TYPE.ORG = "SESSION" THEN ;* GLOBUS_BG_100004988
...
* EN_10001999 E
    ...
    END
    IF OFS$SOURCE.REC<OFS.SRC.LOG.DETAIL.LEVEL> = 'FULL' THEN
        LOG.LEVEL = 4
    END
*
...
*
    BEGIN CASE
    CASE VALIDATE.OR.PROCESS = 'VALIDATE'
        GOSUB RESTORE.CACHE   ;* Validate must put back the old cache
    CASE (SUCCESS.FAIL = 1 AND JOURNAL.BYPASS)  OR WRITE.CACHE        ;* leave the cache as it is.If WRITE.CACHE is set,dont't restore the cache.
...
* Restore Company Details if only it is a diffenrent than the original company
    IF ID.COMPANY # ID.COMPANY.ORG THEN
...
* Restore Browser Vars
    ...
    IF NOT(PW$FOLLOW.ON.ACT) THEN OFS$NEXT.COMMAND = OFS$NEXT.COMMAND.ORG       ;* EN_10002888 S/E
...
*
    ...
    BEGIN CASE
    CASE DEFAULT.RECORD.TYPE = "XML"
        OFS$SOURCE.REC<OFS.SRC.SYNTAX.TYPE> = "XML"
    CASE DEFAULT.RECORD.TYPE = "OFS"
