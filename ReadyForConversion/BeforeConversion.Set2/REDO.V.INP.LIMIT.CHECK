...
*
  R.TELLER.PARAMETER = ""
*
  CALL F.READ(FN.TELLER.PARAMETER,TELL.ID,R.TELLER.PARAMETER,F.TELLER.PARAMETER,TELL.ERR)
  IF R.TELLER.PARAMETER EQ "" THEN
    AF              = TT.TE.AMOUNT.LOCAL.1
    ETEXT           = "TT-RECORD.&.NOT.IN.TELLER.PARAMETER":FM:TELL.ID
    PROCESS.GOAHEAD = ""
    CALL STORE.END.ERROR
  END ELSE
    DESN   = R.TELLER.PARAMETER<TT.PAR.LOCAL.REF><1,LOC.DESGN>
    VCOUNT = DCOUNT(DESN,SM)
  END
*
*----------------------------------------------------------------------------
...
*
  ...
  LOOP
*        I = 1
    LIMIT.DESGN = R.TELLER.PARAMETER<TT.PAR.LOCAL.REF><1,LOC.DESGN,I>
    LIMIT.CCY   = R.TELLER.PARAMETER<TT.PAR.LOCAL.REF><1,LOC.LIMIT.CCY,I>
  WHILE (I LE VCOUNT)
    IF LIMIT.DESGN EQ CASHIER.ROLE AND LIMIT.CCY EQ TRAN.CCY THEN
      LIMIT.AMT = R.TELLER.PARAMETER<TT.PAR.LOCAL.REF><1,LOC.LIMIT.AMT,I>
      TOT.AMT   = TOT.AMT + LIMIT.AMT
*            I         = VCOUNT + 1
    END
...
*
  ...
  TTTX.ERR             = ""
*
  CALL F.READ(FN.TELLER.TRANSACTION,TTTX.ID,R.TELLER.TRANSACTION,F.TELLER.TRANSACTION,TTTX.ERR)
  IF R.TELLER.TRANSACTION EQ "" THEN
    AF              = TT.TE.TRANSACTION.CODE
    ETEXT           = "EB-TELLER.TRANSACTION.&.NOT.DEFINED":FM:TTTX.ID
    PROCESS.GOAHEAD = ""
    CALL STORE.END.ERROR
  END
...
*
  PROCESS.GOAHEAD = 1
*
  LOC.REF.APPLICATION = 'USER' : FM : 'TELLER.PARAMETER' : FM : "TELLER" : FM : "TELLER.TRANSACTION"
  LOC.REF.FIELDS      = 'L.US.CASIER.ROL':FM:'L.TT.DESGN':VM:'L.TT.LIMIT.CCY':VM:'L.TT.LIMIT.AMT':FM:"L.CREDIT.AMOUNT":VM:"L.DEBIT.AMOUNT":FM:"L.CHECK.LIMIT"
  LOC.REF.POS         = ''
*
