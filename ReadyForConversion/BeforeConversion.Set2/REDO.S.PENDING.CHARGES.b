...
*PACS00532228-S
    ...
    ACT.CLOSE.BAL = ACT.CLOSE.BAL - Y.LOCK.AMT
*PACS00532228-E
...
*~~~~~~~~~~~~~~~~~~~~
  ...
  IF R.PENDING.CHARGE THEN
    ...
    GET.CHG.TXN.TOT = GET.CHG.TXN.TOT + 1
  END
  IF UPDATE.CPC THEN
...
*PACS00055362-S
    ...
    R.PENDING.CHARGE<Y.FIN-6> = TNO:"_":OPERATOR  ;* INPUTTER
    X = OCONV(DATE(),"D-")
    R.PENDING.CHARGE<Y.FIN-5> = X[9,2]:X[1,2]:X[4,2]:TIME.STAMP[1,2]:TIME.STAMP[4,2]
    R.PENDING.CHARGE<Y.FIN-4> = TNO:"_":OPERATOR  ;* AUTHORISER
    IF R.PENDING.CHARGE<Y.FIN-3> = "" THEN        ;* COMPANY ID
...
* in REDO.PENDING.CHARGE and  For each record goto RECOVER.ENTRIES
  ...
  IF CALC.OPENING.BALANCE GT 0 THEN
    ...
    WHILE START.CNT.CONT LE CNT.NO.TXN
      ...
      CALC.OPENING.BALANCE  = CALC.OPENING.BALANCE - TEMP.MV.AMT
