...
INI:
    ...
    NEXT A
*    CNT.B = 0
    FOR B = 1 TO T.CANT.VM.REL.CO STEP 1
        RC = R.ACC<AC.RELATION.CODE, A>
        IF (RC EQ 500) OR (RC EQ 501) OR (RC EQ 510) THEN
            T.AC.CUSTOMER.2 = R.ACC<AC.JOINT.HOLDER, B>
            BREAK
        END
*********************************************************************************************************************************
...
*********************************************************************************************************************************
    ...
    GET_ACCOUNT_CAT:
    CALL F.READ(FN.CAT,T.AC.CATEGORY,R.CAT, FN.CAT, CAT.ERR)
...
**DE LO CONTRARIO VERIFICO T.AC.CUSTOMER.2
    ...
    END
    IF T.TIPO.CLIENTE = "PERSONA FISICA" THEN
...
**HAGO UNA PEQUEñA LIMPIEZA A LA VARIABLE DE IDENTIDAD EN CASO DE QUE EL CUSTOMER NO EXISTA...
    ...
    GET_FIRST_COUNTRY:
    CALL F.READ(FN.COU,T.TIT.NACIONALIDAD.1,R.COU, FV.COU, COU.ERR)
    T.COU.COUNTRY.NAME.1 = R.COU<EB.COU.COUNTRY.NAME>

    RETURN

    GET_SECOND_COUNTRY:
    CALL F.READ(FN.COU,T.TIT.NACIONALIDAD.2,R.COU, FV.COU, COU.ERR)
...
*GOSUB GET_CO_CODE
    ...
    GET_CO_CODE:
    CALL F.READ(FN.COM,T.CO.CODE,R.COM, FV.COM, COM.ERR)
