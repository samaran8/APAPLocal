...
$INSERT I_REDO.FX.OVR.COMMON
  ...
  IF V$FUNCTION MATCHES "D":VM:"R" ELSE
    GOSUB INITIALISE
    GOSUB PRE.CHECK
...
*PACS00149084 - S
  APPL.NAME = 'FX.PARAMETERS':FM:'FOREX'
  FLD.NAME = 'L.FX.OVR.TIME':VM:'L.FX.FIR.OVR':FM:'L.FX.BRANCH'
...
*PACS00149084 - S
  ...
  CALL F.READ(FN.COMP,Y.BRN.ID,R.COMP,F.COMP,ERR.COM)
  ...
  CHANGE VM TO ',' IN Y.MAIL.ID
  CHANGE VM TO ' ' IN Y.SUBJECT.BRN
  CHANGE VM TO ' ' IN Y.BODY.BRN
*PACS00149084 - E
  RETURN
...
* Pre check to process only for FXSN number
  ...
  IF R.FXSN.VERSION THEN
    ...
    CHANGE VM TO FM IN Y.FXSN.VERSION
