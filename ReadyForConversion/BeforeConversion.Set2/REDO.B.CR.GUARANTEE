...
* <region name= Inserts>
    ...
    $INSERT T24.BP I_F.COLLATERAL.TYPE
...
*---------------------------
    ...
    FOR I = 1 TO Y.CNT
        Y.L.AC.LK.COL.ID.POS = R.COLLATERAL<COLL.LOCAL.REF,L.AC.LK.COL.ID.POS,I>
        CALL L.APAP.RETURN.BANLACE.CANCELACION(Y.L.AC.LK.COL.ID.POS,OUT.RECORD)
        Y.CAPITAL.PENDIENTE = FIELD(OUT.RECORD,"*",1)
        Y.MONTO.TOTAL = Y.MONTO.TOTAL + Y.CAPITAL.PENDIENTE
...
*-------------------------------------------------------
    ...
    IF Y.L.APAP.GRTA.FID EQ '' AND YNAME.CUST[1,8] = 'FIDEICOM' THEN
        Y.L.APAP.GRTA.FID = "S"
    END
