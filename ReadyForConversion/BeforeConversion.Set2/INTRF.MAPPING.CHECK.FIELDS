...
*
    IF INDEX(R.NEW(INTRF.MAP.OFS.UTIL.NAME),'ENQUIRY.SELECT',1) THEN RETURN     ;* No checks for enquiry temp
    BEGIN CASE
    CASE AS
...
*
    IF R.NEW(INTRF.MAP.APPLICATION) THEN
        ...
        R.SS.APP=''
        CALL F.READ(FN.STANDARD.SELECTION,APP.NAME,R.SS.APP,F.STANDARD.SELECTION,ER.SS)
    END
...
DEFAULT.OTHER.FIELDS:
    ...
    CASE AF EQ INTRF.MAP.OFS.UTIL.NAME
        ...
        IF NOT(R.ENQUIRY.SELECT) THEN
            CALL F.READ(FN.ENQUIRY,COMI,R.ENQUIRY,F.ENQUIRY,ENQUIRY.ERR)
            IF R.ENQUIRY THEN
                ...
                END
            END ELSE
                CALL F.READ(FN.VERSION,COMI,R.COMI.VERSION,F.VERSION,VERSION.ERR)
                IF R.COMI.VERSION THEN
                    FILE.NAME = FIELD(COMI,',',1)
                    CALL F.READ(FN.PGM.FILE,FILE.NAME,R.PGM.FILE,F.PGM.FILE,PF.ERR)
                    IF R.PGM.FILE THEN
...
CHECK.INPUT:
    ...
    CASE R.NEW(INTRF.MAP.FIELD.SOURCE)<1,AV> EQ 'RTN'
        CALL F.READ(FN.PGM.FILE,COMI,R.PGM.FILE,F.PGM.FILE,PF.ERR)
