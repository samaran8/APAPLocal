...
* Change for KCB \e
        END
    CASE AF = ATM.BR.DEV.ID.INT.ACCT
...
*
    ...
    CASE AF = ATM.BR.UTILITY.NAME
        IF R.NEW(ATM.BR.UTIL.NO)<1,AV> AND COMI = '' THEN
            E = 'INPUT MISSING'
        END
    CASE AF = ATM.BR.UTIL.NO
        IF R.NEW(ATM.BR.UTILITY.NAME)<1,AV> AND COMI = '' THEN
            E = 'INPUT MISSING'
        END
    CASE AF = ATM.BR.UTIL.CATEG
        IF COMI THEN
            ...
            END
        END
    CASE AF = ATM.BR.UTIL.AC.SUFX
        IF COMI AND R.NEW(ATM.BR.UTIL.INT.ACCT)<1, AV> THEN
            E = 'COMBINATION OF DEV.ID.CATEG AND DEV.ID.AC.SUFX OR DEV.ID.INT.ACCT ONLY ALLOWED'
        END ELSE
            IF R.NEW(ATM.BR.UTIL.CATEG)<1,AV> AND COMI = '' THEN
                E = 'INPUT MISSING'
            END ELSE
                IF COMI AND LEN(COMI) NE 4 THEN
                    E = 'MUST BE LENGTH OF 4'
...
CHECK.COMPANY.CODE:
    ...
    FOR Y.CTR = 1 TO Y.NO.OF.REC
        ...
        IF COMI = Y.COMPANY.CODE AND Y.ATM.BR.LIST<Y.CTR> NE ID.NEW THEN
