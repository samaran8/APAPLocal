...
*    CALL OCOMO('Y.REQUEST ':Y.REQUEST )
    ...
    CALL REDO.COL.R.CALL.JAVA.API("REDO.COL.DB.INSERT", Y.REQUEST, Y.RESPONSE)
...
* Rollback
        ...
        Y.RESPONSE1    = ''
        Y.REQUEST     = ''
        CALL REDO.COL.R.CALL.JAVA.API(Y.EB.API.ID, Y.REQUEST, Y.RESPONSE1)
...
*INVOCACION A REDO.R.COL.PROCESS.TRACE con status = 10
            CALL REDO.R.COL.PROCESS.TRACE('DELIVERY', 10, Y.CONTADOR + X1, Y.TABLE.ID, Y.DETAILS.REC<2,X1>, '')
        NEXT X1
    END
...
*-----------------------------------------------------------------------
    IF Y.MUST.DELETE THEN
        CALL REDO.COL.R.DEL.UPD.LOCKING("DELETE",Y.TABLE.ID)
...
*INVOCACION A REDO.R.COL.PROCESS.TRACE con status = 20
    CALL REDO.R.COL.PROCESS.TRACE('DELIVERY', 20, Y.CONTADOR + Y.VM.POS, Y.TABLE.ID, Y.DETAIL, C.DESC)
    CALL REDO.R.COL.EXTRACT.ERROR(C.DESC, C.ID.PROC,Y.TABLE.ID)
