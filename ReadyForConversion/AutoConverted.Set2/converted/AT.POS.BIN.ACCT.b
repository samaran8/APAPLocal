...
*-----------------------------------------------------------------------------
*-----------------------------------------------------------------------------*
* Application to store the POS details BIN wise
*-----------------------------------------------------------------------------*

      SUBROUTINE AT.POS.BIN.ACCT
...
* > CALL XX.CHECK.FIELDS
      ...
      BEGIN CASE
         ...
         CASE AF EQ AT.PBA.PL.COM.CATEG

            IF (COMI) THEN

               IF LEN(COMI) EQ '5' AND NUM(COMI) THEN
                  CALL F.READ(FN.CATEGORY,COMI,CAT.REC,F.CATEGORY,CAT.RERR)
                  IF CAT.REC THEN
                     COMI.ENRI = CAT.REC<EB.CAT.DESCRIPTION>
                  END ELSE
                     E = 'INVALID CATEGORY'
                  END
               END ELSE

                  CALL F.READ(FN.FT.COMMISSION.TYPE,COMI,COM.REC,F.FT.COMMISSION.TYPE,AC.RERR)
                  IF COM.REC THEN
                     COMI.ENRI = COM.REC<FT4.DESCRIPTION>
                  END ELSE
                     E = 'INVALID COMMISSION TYPE'
                  END
...
DEFINE.PARAMETERS:* SEE 'I_RULES' FOR DESCRIPTIONS *
      ...
      CALL AT.POS.BIN.FIELD.DEFINITIONS
