* @ValidationCode : MjoxNDgxNjg0OTI2OkNwMTI1MjoxNjgyMDAwMTY4NzE2OklUU1M6LTE6LTE6NTI2OjE6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 20 Apr 2023 19:46:08
...
*-------------------------------------
    ...
    WHILE Y.VAR1 LE FIELDS.CNT
        ...
        Y.VAR1 += 1
    REPEAT
    CALL APAP.REDOENQ.REDO.E.FORM.SEL.STMT(FN.APAP.H.INSURANCE.DETAILS, '', '', SEL.CMD);* R22 Manual Conversion - CALL method format changed
...
*-------------------------------------
    ...
    IN.ACC.ID = ''
    OUT.ID = ''
    CALL APAP.AA.REDO.CONVERT.ACCOUNT(IN.ACC.ID,Y.AA.ID,OUT.ID,ERR.TEXT);* R22 Manual Conversion - CALL method format changed
    GOSUB GET.PREVIOUS.LOAN.NO
    GOSUB GET.CUSTOMER.DETAILS
    Y.CLOSING.DATE = R.APAP.H.INSURANCE.DETAILS<INS.DET.POL.EXP.DATE>
    Y.DISB.AMT = 0
    CALL APAP.TAM.REDO.GET.DISBURSED.AMT(Y.AA.ID,Y.DISB.AMT) ;* R22 Manual Conversion - CALL method format changed
    ...
    CALL APAP.TAM.REDO.GET.TOTAL.OUTSTANDING(Y.AA.ID,Y.PROP.AMT,Y.TOTAL.AMT) ;* R22 Manual Conversion - CALL method format changed
...
*-------------------------------------
    ...
    R.ACC.CONDITION = ''
    ERR.MSG         = ''
    CALL APAP.AA.REDO.CRR.GET.CONDITIONS(Y.AA.ID,EFF.DATE,PROP.CLASS,PROPERTY,R.ACC.CONDITION,ERR.MSG);* R22 Manual conversion - CALL method format changed
