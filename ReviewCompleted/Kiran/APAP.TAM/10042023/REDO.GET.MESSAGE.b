...
*----------------------------------------------------------------------------
    ...
    Y.ARR.ID = ''
    Y.ACC.NO = VAR.AC.ID
    CALL APAP.TAM.REDO.CONVERT.ACCOUNT(VAR.AC.ID,Y.ARR.ID,OUT.ID,ERR.TEXT) ;*MANUAL R22 CODE CONVERSION
    VAR.AA.ID = OUT.ID
...
* Here we form the message for deal slip
    ...
    CALL APAP.TAM.REDO.GET.NEXT.PAYMENT.AMOUNT(VAR.AA.ID,TODAY,Y.NEXT.PAY.AMT) ;*MANUAL R22 CODE CONVERSION
    ...
    Y.FMT.MSG.ARCIB = ''
    CALL APAP.TAM.REDO.FORMAT.MESSAGE(VAR.AC.ID,Y.ORD.MESSAGE,Y.ADD.VALUE,Y.FMT.MSG.ARCIB) ;*MANUAL R22 CODE CONVERSION
