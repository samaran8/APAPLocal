* @ValidationCode : MjoxOTY0MTY4MzYxOkNwMTI1MjoxNjgzMjg2OTM0MTgzOmhhaTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 05 May 2023 17:12:14
...
*---------------------------------------------------------
    ...
    CALL APAP.TAM.redoGetNextPaymentAmount(Y.ARR.ID,Y.EFFECTIVE.DATE,Y.NEXT.PAY.AMT)
    ...
    CALL APAP.TAM.redoFormatMessage(Y.MSG.ARCIB,Y.ORD.ARCIB,Y.ADD.VALUE,Y.FMT.MSG.ARCIB)
    CALL APAP.TAM.redoFormatMessage(Y.MSG.EMAIL,Y.ORD.EMAIL,Y.ADD.VALUE,Y.FMT.MSG.EMAIL)
...
*----------------------------------------------------------------------
    ...
    IF Y.CURR.PROPERTY EQ 'PRINCIPALINT' THEN
        ...
        CALL APAP.TAM.redoGetNextPaymentAmountOld(Y.ARR.ID,Y.EFFECTIVE.DATE,Y.OLD.PAYMENT.AMOUNT)
        Y.OLD.INT.RATE   = TRIMB(FMT(R.OLD(AA.INT.EFFECTIVE.RATE)<1,1>,'L2,#10'))
        Y.NEW.INT.RATE   = TRIMB(FMT(R.NEW(AA.INT.EFFECTIVE.RATE)<1,1>,'L2,#10'))
