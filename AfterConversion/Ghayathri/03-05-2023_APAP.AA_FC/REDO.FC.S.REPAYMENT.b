* @ValidationCode : MjotMTU3MDgzMjU5MzpDcDEyNTI6MTY4MzEwMjIxNTg0ODpoYWk6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 03 May 2023 13:53:35
...
* PACS00350509 - 2014SEP24 - E
            ...
            IF R.REDO.FC.CL.BALANCE THEN
                CALL APAP.AA.redoFcClPaymentAa(TOTAL.ACC.AMT, c_aalocArrId)
            END ELSE      ;* For Migrated contracts
                CALL APAP.AA.redoFcClPaymentAa(TOTAL.ACC.AMT, c_aalocArrId)       ;* PACS00350509 - S/E
            END
*
        END ELSE
            CALL F.READ(FN.REDO.FC.CL.BALANCE,c_aalocArrId,R.REDO.FC.CL.BALANCE,F.REDO.FC.CL.BALANCE,YERR)
            IF R.REDO.FC.CL.BALANCE THEN
                CALL APAP.AA.redoFcSPaymentAa()
            END ELSE      ;* For Migrated contracts
                CALL APAP.AA.redoFcSPaymentAa()       ;* PACS00350509 - S/E
            END
...
*----------------------------------------------------------------------------------------------------
    ...
    CALL APAP.TAM.redoConvertAccount(IN.ACC.ID,Y.ARR.ID,Y.ACCOUNT.ID,ERR.TEXT)
