* @ValidationCode : MjotMTUwNTU5MTUwMzpDcDEyNTI6MTY4MzExMDA5NzU3NTpoYWk6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 03 May 2023 16:04:57
...
*----------------------------------------------------------------------
    ...
    CALL APAP.AA.redoGetTotalInterest(Y.ARRANGEMENT.ID,PROPERTY,INT.AMT)
    ...
    IF c_aalocPropClassId EQ 'INTEREST' THEN
        ...
        END
    END ELSE
        CALL APAP.AA.redoCrrGetConditions(Y.ARRANGEMENT.ID,'','INTEREST',PROPERTY,R.INT.COND,RET.ERR)
...
* This part gets the principal interest property of that arrangement.
    ...
    OUT.PROP=''
    CALL APAP.TAM.REDO.GET.INTEREST.PROPERTY(Y.ARRANGEMENT.ID,PROP.NAME,OUT.PROP,ERR)
    PROPERTY=OUT.PROP
RETURN
...
*---------------------------------------------------------------------------
    ...
    CALL APAP.AA.REDO.AA.BUILD.OFS(c_aalocArrId,R.INT.CONDITION,Y.PROP.ACTIVITY,OFS.MSG)
