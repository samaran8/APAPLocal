* @ValidationCode : MjotMTA5MjYwNDMxMjpDcDEyNTI6MTY4MzA5OTg4Mjk0MzpoYWk6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 03 May 2023 13:14:42
...
*---------------------------------------------------------------------------
    ...
    IN.ACC.ID = ""
    CALL APAP.TAM.redoConvertAccount(IN.ACC.ID,Y.AA.ID,Y.AC.ID,ERR.TEXT)
...
*---------------------------------------------------------------
    ...
    ERR.MSG     = ''
    R.OVERDUE.COND   = ''
    CALL APAP.AA.redoCrrGetConditions(Y.AA.ID,EFFEC.DATE,PROP.CLASS,PROPERTY,R.OVERDUE.COND,ERR.MSG)
...
*--------------------------------------------------------
    ...
    IF Y.INTERNAL.ACC THEN
        ...
        CALL APAP.TAM.redoNabCreateIntAcc(Y.LOAN.CO.CODE,Y.INTERNAL.ACC,Y.RET.INT.ACC)
