* @ValidationCode : MjozNDU0MzU0NDY6Q3AxMjUyOjE2ODMxMDcxMjc0NTc6aGFpOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 03 May 2023 15:15:27
...
*****Manual R22 Code Conversion*****
        LOCATE c_aalocCurrActivity IN Y.PAYOFF.ACTIVITY<1,1> SETTING POS.PA THEN
            ...
            IF c_aalocActivityStatus MATCHES 'UNAUTH':@VM:'UNAUTH-CHG' THEN ;*AUTO R22 CODE CONVERSION
                CALL APAP.AA.redoPostPayoffRemPaysch()  ;* This routine to remove the scheduled charge after payoff.
            END
...
*-----------------------------------------------------------
    ...
    ERR.MSG = ''
    VAR.ID = c_aalocArrId
    CALL APAP.AA.redoCrrGetConditions(VAR.ID,EFF.DATE,PROP.CLASS,PROPERTY,R.CONDITION.TERM.AMOUNT,ERR.MSG)
...
*-----------------------------------------------------------------
    ...
    WHILE Y.VAR1 LE Y.CHARGE.PROP.CNT
        ...
        R.CONDITION.CHARGE = ''
        ERR.MSG = ''
        CALL APAP.AA.redoCrrGetConditions(ARR.ID,EFF.DATE,PROP.CLASS,PROPERTY,R.CONDITION.CHARGE,ERR.MSG)
        Y.POLICY.NO = ''
