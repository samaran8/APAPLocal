* @ValidationCode : MjotMTU2NDI1MzQzNjpDcDEyNTI6MTY4MzEwMjAzNDgxNzpoYWk6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 03 May 2023 13:50:34
...
*   $INSERT I_F.ACCT.ACTIVITY
    ...
    IF c_aalocActivityStatus EQ 'AUTH' OR c_aalocActivityStatus EQ 'REVERSE' OR c_aalocActivityStatus EQ 'AUTH-REV' THEN
        ...
        IF Y.PROPERTY.AMT AND Y.PROPERTY.AMT GT 0 THEN
            ...
            END
            CALL APAP.AA.redoFcClPaymentAa(Y.PROPERTY.AMT, ID.ARR)
        END
...
**
    ...
    CALL APAP.TAM.redoConvertAccount(IN.ACC.ID,ID.ARR,Y.ACCOUNT.ID,ERR.TEXT)
    ...
    IF PAY.AMOUNT.BALANCE THEN
        IF Y.AMOUNT GE PAY.AMOUNT.BALANCE  THEN
            ...
            IF Y.VALUE.CAPITAL GT 0 THEN ;*AUTO R22 CODE CONVERSION
                CALL APAP.AA.redoFcClPaymentAa(Y.VALUE.CAPITAL, ID.ARR)
            END ELSE
                CALL APAP.AA.redoFcClPaymentAa(Y.AMOUNT, ID.ARR)
            END
        END ELSE
            CALL APAP.AA.redoFcClPaymentAa(Y.AMOUNT, ID.ARR)
        END
