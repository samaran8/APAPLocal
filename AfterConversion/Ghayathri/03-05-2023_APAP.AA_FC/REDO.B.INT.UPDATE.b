* @ValidationCode : MjozODAzODQ1NzU6Q3AxMjUyOjE2ODMwOTg1MDQ2NjY6aGFpOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 03 May 2023 12:51:44
...
*----------------------------------------------------------------------------
    ...
    CALL APAP.TAM.redoGetHighAci(Y.ACI.ID,Y.ACI.CUR,Y.RATE.ACI)
...
*----------------------------------------------------------------------------
    ...
    CALL APAP.TAM.redoGetHighGci(Y.GCI.ID,Y.GCI.CUR,Y.RATE.GCI)
...
*----------------------------------------------------------------------------
    ...
    IF Y.FINAL.RATE NE Y.CURRENT.RATE THEN
        ...
        CALL APAP.AA.redoAaBuildOfs(ARR.ID,R.PRIN.INT.COND.OFS,Y.ACT.PROP,PRIN.OFS.MSG)
...
*----------------------------------------------------------------------------
    ...
    IF Y.FINAL.RATE NE Y.PENAL.CURRENT.RATE THEN
        ...
        CALL APAP.AA.redoAaBuildOfs(ARR.ID,R.PENAL.INT.COND.OFS,Y.ACT.PROP,PENAL.OFS.MSG)
...
*----------------------------------------------------------------------------
    ...
    R.PRIN.INT.COND=''
    ERR.MSG     = ''
    CALL APAP.AA.redoCrrGetConditions(ARR.ID,EFF.DATE,PROP.CLASS,PRINCIPAL.PROP,R.PRIN.INT.COND,ERR.MSG)
    ...
    R.PENAL.INT.COND=''
    ERR.MSG = ''
    CALL APAP.AA.redoCrrGetConditions(ARR.ID,EFF.DATE,PROP.CLASS,PENALTY.PROP,R.PENAL.INT.COND,ERR.MSG)
