* @ValidationCode : MjotOTA2Njg1MjAwOkNwMTI1MjoxNjgzMTAyMTA5NTEzOmhhaTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 03 May 2023 13:51:49
...
* Call routine to populate COLLATERAL.RIGHT
    CALL APAP.REDOFCFI.redoFcSPopCollRight(Y.SEC.NO, Y.I)
RETURN
*------------------------------------------------------------------------------------------------------------------
GET.LOCAL.FIELD:
*------------------------------------------------------------------------------------------------------------------
...
* Getting current AA Balance value
    ...
    Y.AA.BAL = AA.ARR
    CALL APAP.AA.redoSFcAaBal(Y.AA.ID, Y.AA.BAL)
...
* Getting current AA Balance value
    Y.AA.AMOUNT = ''
    Y.AA.AMOUNT = AA.ARR
    CALL APAP.AA.redoSFcAaAmount(Y.AA.ID, Y.AA.AMOUNT)
