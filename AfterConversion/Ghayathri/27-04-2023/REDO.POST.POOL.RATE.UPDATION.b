* @ValidationCode : MjoxMTkyNjExMjE5OkNwMTI1MjoxNjgyNTkyMDYyMDE2OmhhaTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 27 Apr 2023 16:11:02
...
*-----------------------------------------------------------------------------
    ...
    IF (OFS$BROWSER OR RUNNING.UNDER.BATCH) AND (c_aalocArrActivityRec<AA.ARR.ACT.ACTIVITY.CLASS> NE 'LENDING-NEW-ARRANGEMENT' AND c_aalocArrActivityRec<AA.ARR.ACT.ACTIVITY.CLASS> NE 'LENDING-TAKEOVER-ARRANGEMENT') AND Y.FREQ.REVIEW NE '' THEN
        ...
        Y.OUT.DATE = ''
        CALL APAP.TAM.RedoGetNextCycledate(ARR.ID,FREQ,Y.DATE,Y.OUT.DATE)
* PACS00585816 - start
...
*
    ...
    IF Y.REVIEW.FREQ NE '' THEN
        Y.RATE = ''
        CALL APAP.TAM.redoGetPoolRate(Y.CURRENCY,Y.REVIEW.FREQ,Y.RATE)
        IF Y.RATE<1> THEN
            Y.FLAG = 1
            GOSUB MULTI.IF
        END
...
*---------------------------------------------------
    ...
    R.CONDITION  = ''
    ERR.MSG      = ''
    CALL APAP.AA.REDO.CRR.GET.CONDITIONS(c_aalocArrId,EFF.DATE,PROP.CLASS,PROPERTY,R.CONDITION,ERR.MSG)
...
* We need to assign pool rate based on the lowest deposit term
    ...
    IF YDAYS THEN
        ...
        Y.AZ.RATE = ''
        CALL APAP.TAM.redoGetPoolRate(Y.CURRENCY,Y.REVIEW.FREQ,Y.AZ.RATE)
...
*---------------------------------------------------------
    ...
    Y.RATE.VALUE  = ''
    CALL APAP.TAM.redoGetPoolRate(Y.CURRENCY,Y.REVIEW.FREQ,Y.RATE.VALUE)
...
* Get the Term of the Loan
    ...
    CALL APAP.TAM.redoGetLoanTerm(c_aalocArrId,c_aalocActivityEffDate,Y.LOAN.TERM)
