* @ValidationCode : MjotMTkyMDk3OTQ6Q3AxMjUyOjE2ODI1ODM2MzczODc6aGFpOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 27 Apr 2023 13:50:37
...
*-----------------------------------------------------------
    ...
    CALL REDO.GET.INTEREST.PROPERTY(Y.AA.ID,PROP.NAME,Y.PRIN.PROP,ERR);*Manual R22 conversion
    PROP.NAME      = 'PENALTY'
    Y.PENALTY.PROP = ''
    ERR            = ''
    CALL REDO.GET.INTEREST.PROPERTY(Y.AA.ID,PROP.NAME,Y.PENALTY.PROP,ERR);*Manual R22 conversion
...
* We need to assign pool rate based on the lowest deposit term.
    ...
    IF YDAYS THEN
        ...
        Y.AZ.RATE = ''
        CALL APAP.AA.REDO.GET.POOL.RATE(Y.CURRENCY,Y.REVIEW.FREQ,Y.AZ.RATE)
